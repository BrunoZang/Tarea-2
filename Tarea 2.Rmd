...
gastos %>%
  ggplot(aes(Edad, fill = Sexo))+geom_histogram()+facet_wrap( ~ Sexo)+labs(x =  "Edad", y = "Número de pacientes", caption="Se puede ver que hay mas pacientes del Sexo Masculino en la mayoria de las edades y se puede ver un incremento de pacientes en ambos sexo empezando de los 50 en adelante")
```
...
gastos %>%
  ggplot(aes(x = fct_infreq(Prestacion))) +geom_bar()+coord_flip() +labs(y = "Número de pacientes", x = "Prestaciones", caption="Se puede visualizar que la prestacion que mas se realiza es PCI")
```

```{r}
ggplot(gastos, aes(x=Prestacion,fill=Sexo))+
geom_bar()+coord_flip()+facet_wrap(~Sexo)+
labs(x="Tipo de prestaciones",y="Número de pacientes",caption="Para los dos sexos la prestacion que mas se realiza es PCI")
```



 3. ¿Donde ocurre la mayoria de los casos y en donde esta la mayoria de los pacientes?
```{r}
gastos %>% ggplot(aes(x = fct_infreq(Prestador_departamento))) +geom_bar() +coord_flip() +
  labs(x = "Departamento", y = "Número de pacientes",caption="Donde ocurren mas casos es en Montevideo predecible porque mas de la mitad de la poblacion uruguaya vive en la capital")
```
...
```{r}
group_by(gastos,Edad,Sexo) %>%summarise(Media=sum(Importe))%>% ggplot(aes(Edad,Media,colour=Sexo))+geom_line()+labs(x="Edad", y="Importe Total",caption="Se puede ver que a temprana edad Mujeres pagan un poco mas que los hombres hasta una edad de 45 años donde el Hombre supera los gastos de la mujer, pero luego a una avanzada edad de 85 años nuevamente las mujeres pagan un poco mas que los hombres")
```

 5. Se me ocurrira despues.
